...
class Menu_InGame_Manage_Info extends SliderMenu {
   ...
   protected Menu_InGame_Manage_Info() {
      ...
      menuElements.add(new Text_Scrollable((String)null, CFG.PADDING * 4, CFG.PADDING * 3, CFG.CIV_INFO_MENU_WIDTH - CFG.PADDING * 5, CFG.COLOR_TEXT_CIV_NAME) {
         ...
         protected void draw_Element(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive, boolean scrollableY) {
            if (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getCivID() != CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()) {
               super.draw_Element(oSB, iTranslateX + CFG.PADDING + (int)((float)CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getWidth() * Menu_InGame_Manage_Info.this.getImageScale(CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight())), iTranslateY, isActive, scrollableY);
               CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().draw(oSB, this.getPosX() + this.getCurrent() + iTranslateX, this.getPosY() - CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight() + (int)((float)this.getHeight() - (float)CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight() * Menu_InGame_Manage_Info.this.getImageScale(CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight())) / 2 + iTranslateY, (int)((float)CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getWidth() * Menu_InGame_Manage_Info.this.getImageScale(CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight())), (int)((float)CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight() * Menu_InGame_Manage_Info.this.getImageScale(CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight())));
               ImageManager.getImage(Images.flag_rect).draw(oSB, this.getPosX() + this.getCurrent() + iTranslateX, this.getPosY() - ImageManager.getImage(Images.flag_rect).getHeight() + (int)((float)this.getHeight() - (float)CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight() * Menu_InGame_Manage_Info.this.getImageScale(CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight())) / 2 + iTranslateY, (int)((float)CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getWidth() * Menu_InGame_Manage_Info.this.getImageScale(CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight())), (int)((float)CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight() * Menu_InGame_Manage_Info.this.getImageScale(CFG.game.getCiv(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getPuppetOfCivID()).getFlag().getHeight())));
            } else {
      ...
      menuElements.add(new Text((String)null, CFG.PADDING * 4, CFG.PADDING * 4 + CFG.TEXT_HEIGHT) {
         ...
         protected void buildElementHover() {
            this.menuElementHover = CFG.game.getHover_LeaderOfCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
         }
      });
