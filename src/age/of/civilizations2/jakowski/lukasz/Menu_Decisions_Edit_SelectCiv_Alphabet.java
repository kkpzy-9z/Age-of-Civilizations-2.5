...
class Menu_Decisions_Edit_SelectCiv_Alphabet extends SliderMenu {
    private List<Character> lCharacters;
    private String nSearch = null;

    protected Menu_Decisions_Edit_SelectCiv_Alphabet() {
        ...
        menuElements.add(new Button_Menu_Classic_Search("", CFG.PADDING * 2, 0, CFG.PADDING, CFG.BUTTON_WIDTH * 2, CFG.BUTTON_HEIGHT, true) {
            ...
            protected String getTextToDraw() {
                return Menu_Decisions_Edit_SelectCiv_Alphabet.this.nSearch + ": " + super.getTextToDraw();
            }
        });
        ...
        for (String s : tagsSPLITED) {
            if (!CFG.isInDecisionCivs(s)) {
                ...
                for (int a = 0; a < this.lCharacters.size(); ++a) {
                    if (this.lCharacters.get(a) == CFG.langManager.getCiv(s).charAt(0)) {
                        addChar = false;
                        break;
                    }
                }
