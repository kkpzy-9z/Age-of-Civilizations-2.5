...
extends SliderMenu {
    ...
    protected Menu_InGame_ChangeGovernment(int nID) {
        ...
        for (int i = 0; i < CFG.ideologiesManager.getIdeologiesSize(); ++i) {
            ...
            tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
            ((MenuElement)menuElements.get(menuElements.size() - 1)).setClickable((canChangeToIdeology.get(i)) || CFG.SPECTATOR_MODE);
        }
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, tY, CFG.BUTTON_WIDTH, true){ //make not clickable if trial for civil war
            ...
            protected boolean getClickable() {
                return (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMovePoints() >= 22 && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getMoney() >= (long)Ideologies_Manager.getChangeGovernmentCost(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) && CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getTechnologyLevel() >= 0.4f && (!CFG.DYNAMIC_EVENTS || !CFG.dynamicEventManager.eventManagerCivilWar.inCivilWar(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()))) || CFG.SPECTATOR_MODE;
            }
            ...
            if (toIdeology >= 0) {
                if (CFG.SPECTATOR_MODE) {
                    if (CFG.ideologiesManager.getIdeology((int)CFG.game.getCiv((int)CFG.game.getPlayer((int)CFG.PLAYER_TURNID).getCivID()).getIdeologyID()).CAN_BECOME_CIVILIZED >= 0) {
                        CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).setCivTag(CFG.ideologiesManager.getRealTag(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getCivTag()) + CFG.ideologiesManager.getIdeology(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getIdeologyID()).getExtraTag());
                        CFG.unionFlagsToGenerate_Manager.addFlagToLoad(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
                        ...
                        for(int i3 = 0; i3 < CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getNumOfProvinces(); ++i3) {
                            CFG.game.getProvince(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getProvinceID(i3)).setFromCivID(0);
                        }

                        CFG.palletManager.loadCivilizationStandardColor(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
                        if (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getNumOfNeighboringNeutralProvinces() > 0) {
                            List<Integer> possibleProvinces = new ArrayList();
                            for(int i4 = 0; i4 < CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getNumOfProvinces(); ++i4) {
                                for(int j = 0; j < CFG.game.getProvince(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getProvinceID(i4)).getNeighboringProvincesSize(); ++j) {
                                    possibleProvinces.add(CFG.game.getProvince(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getProvinceID(i4)).getNeighboringProvinces(j));
                                }
                            }
                            if (possibleProvinces.size() > 0) {
                                CFG.game.getProvince((Integer)possibleProvinces.get(CFG.oR.nextInt(possibleProvinces.size()))).setCivID(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), false);
                            }
                        }
                    }

                    CFG.game.updateCivilizationIdeology(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), CFG.ideologiesManager.getRealTag(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getCivTag()) + CFG.ideologiesManager.getIdeology(toIdeology).getExtraTag());
                    for(int i2 = 0; i2 < CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getCivRegionsSize(); ++i2) {
                        CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getCivRegion(i2).buildScaleOfText();
                    }
                ...
                } else if (DiplomacyManager.changeGovernmentType(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), toIdeology)) {
                    ArrayList<String> tMess = new ArrayList<String>();
                    ArrayList<Color> tColor = new ArrayList<Color>();
                    tMess.add(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getCivName());
                    tColor.add(CFG.ideologiesManager.getIdeology(CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getIdeologyID()).getColor());
