...
class Menu_InGame_ChangeAutonomy
extends SliderMenu {
    protected int iCivID;
    protected Menu_InGame_ChangeAutonomy() {
        ...
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("Autonomy"), CFG.BUTTON_HEIGHT * 3 / 5, true, true), CFG.GAME_WIDTH / 2 - tempWidth / 2, tempMenuPosY, tempWidth, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING + tempMenuPosY > CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING * 2 ? Math.max(CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT + CFG.PADDING * 2) * 6) : ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, menuElements, false, true);
        this.updateLanguage();
    }

    protected Menu_InGame_ChangeAutonomy(int iID) {
        this.iCivID = iID;
        ArrayList<MenuElement> menuElements = new ArrayList<MenuElement>();
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Build(CFG.langManager.get("ChangeAutonomyLevel"), Images.diplo_lord, 0, 0, 2, tY, CFG.BUTTON_WIDTH * 2, false, false, 0, 0.0F){
            @Override
            protected int getWidth() {
                return Menu_InGame_ChangeAutonomy.this.getElementW() * 2;
            }
        });
        tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
        //List<Boolean> canChangeToIdeology = CFG.ideologiesManager.canChangeToIdeologyCFG.game.getProvince((CFG.game.getActiveProvinceID().getCivID()));
        tY += CFG.PADDING;
        int j = 0;
        for (int i = 0; i < CFG.gameAutonomy.lAutonomy.size(); ++i) {
            menuElements.add(new Button_Diplomacy_ChangeAutonomy(j++, this.iCivID, i, 2, tY, CFG.BUTTON_WIDTH * 2){
            ...
            tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
            ((MenuElement)menuElements.get(menuElements.size() - 1)).setClickable((CFG.gameAutonomy.canChangeToAutonomy(this.iCivID).get(i)));
        }
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Cancel"), -1, 2 + CFG.PADDING, tY += CFG.PADDING, CFG.BUTTON_WIDTH, true){
            ...
            @Override
            protected int getWidth() {
                return Menu_InGame_ChangeAutonomy.this.getElementW() - CFG.PADDING - CFG.PADDING / 2;
            }
        });
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("Confirm"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
            ...
            protected int getPosX() {
                return Menu_InGame_ChangeAutonomy.this.getElementW() + CFG.PADDING / 2;
            }
            ...
            protected void buildElementHover() {
                ...
                nData.add(new MenuElement_Hover_v2_Element_Type_Flag(Menu_InGame_ChangeAutonomy.this.iCivID));
                nData.add(new MenuElement_Hover_v2_Element_Type_Text(CFG.langManager.get("ChangeAutonomyLevel"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
            ...
            protected boolean getClickable() {
                return true; //(CFG.game.getCivCFG.game.getProvince((CFG.game.getActiveProvinceID().getCivID())).getMovePoints() >= 22 && CFG.game.getCivCFG.game.getProvince((CFG.game.getActiveProvinceID().getCivID())).getMoney() >= (long)Ideologies_Manager.getChangeGovernmentCostCFG.game.getProvince((CFG.game.getActiveProvinceID().getCivID())) && CFG.game.getCivCFG.game.getProvince((CFG.game.getActiveProvinceID().getCivID())).getTechnologyLevel() >= 0.4f && (!CFG.DYNAMIC_EVENTS || !CFG.dynamicEventManager.eventManagerCivilWar.inCivilWarCFG.game.getProvince((CFG.game.getActiveProvinceID().getCivID())))) || CFG.SPECTATOR_MODE;
            }
        ...
        int tempMenuPosY = ImageManager.getImage(Images.top_left2).getHeight() + CFG.PADDING * 2 + CFG.BUTTON_HEIGHT * 3 / 5;
        this.initMenu(new SliderMenuTitle(CFG.langManager.get("Autonomy"), CFG.BUTTON_HEIGHT * 3 / 5, true, true){
            ...
            protected void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                oSB.setColor(new Color((float)CFG.game.getCiv(Menu_InGame_ChangeAutonomy.this.iCivID).getR() / 255.0f, (float)CFG.game.getCiv(Menu_InGame_ChangeAutonomy.this.iCivID).getG() / 255.0f, (float)CFG.game.getCiv(Menu_InGame_ChangeAutonomy.this.iCivID).getB() / 255.0f, 0.165f));
                ...
                oSB.setColor(new Color((float)CFG.game.getCiv(Menu_InGame_ChangeAutonomy.this.iCivID).getR() / 255.0f, (float)CFG.game.getCiv(Menu_InGame_ChangeAutonomy.this.iCivID).getG() / 255.0f, (float)CFG.game.getCiv(Menu_InGame_ChangeAutonomy.this.iCivID).getB() / 255.0f, 0.375f));
                ...
                CFG.game.getCiv(Menu_InGame_ChangeAutonomy.this.iCivID).getFlag().draw(oSB, Menu_InGame_ChangeAutonomy.this.getPosX() + CFG.PADDING * 2 + iTranslateX, Menu_InGame_ChangeAutonomy.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.game.getCiv(Menu_InGame_ChangeAutonomy.this.iCivID).getFlag().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
                ImageManager.getImage(Images.flag_rect).draw(oSB, Menu_InGame_ChangeAutonomy.this.getPosX() + CFG.PADDING * 2 + iTranslateX, Menu_InGame_ChangeAutonomy.this.getPosY() - this.getHeight() / 2 - CFG.CIV_FLAG_HEIGHT / 2);
                CFG.fontMain.getData().setScale(0.8f);
        ...
        }, CFG.GAME_WIDTH / 2 - tempWidth / 2, (CFG.GAME_HEIGHT - (((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING + tempMenuPosY > CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING * 2 ? Math.max(CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT + CFG.PADDING * 2) * 6) : ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING))/2, tempWidth, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING + tempMenuPosY > CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING * 2 ? Math.max(CFG.GAME_HEIGHT - CFG.BUTTON_HEIGHT - CFG.PADDING * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT + CFG.PADDING * 2) * 6) : ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, menuElements, true, true);
        this.updateLanguage();
        ...
        if (iID == this.getMenuElementsSize() - 1) {
            ...
            if (toIdeology >= 0) {
                if (CFG.SPECTATOR_MODE) {
                    int iLord = CFG.game.getCiv(this.iCivID).getPuppetOfCivID();
                    CFG.game.getCiv(iLord).removeVassal(this.iCivID);
                    CFG.game.getCiv(this.iCivID).setPuppetOfCivID(iLord, toIdeology);

                    for (i = 0; i < CFG.game.getCiv(this.iCivID).getCivRegionsSize(); ++i) {
                        CFG.game.getCiv(this.iCivID).getCivRegion(i).buildScaleOfText();
                    }

                    CFG.toast.setInView(CFG.langManager.get(CFG.gameAutonomy.getAutonomy(toIdeology).getName()), CFG.COLOR_INGAME_GOLD);
                } else if (CFG.menuManager.getInManageDiplomacy()) {
                    int iLord = CFG.game.getCiv(this.iCivID).getPuppetOfCivID();
                    for(int k = 0; k < CFG.game.getCiv(iLord).civGameData.lVassals.size(); ++k) {
                        if (((Vassal_GameData)CFG.game.getCiv(iLord).civGameData.lVassals.get(k)).iCivID == this.iCivID) {
                            //return index after searching
                            ((Vassal_GameData)CFG.game.getCiv(iLord).civGameData.lVassals.get(k)).autonomyStatus = CFG.gameAutonomy.getAutonomy(toIdeology);
                            break;
                        }
                    }
                    CFG.game.getCiv(this.iCivID).loadFlag();
                    for (i = 0; i < CFG.game.getCiv(CFG.game.getProvince(CFG.game.getActiveProvinceID()).getCivID()).getNumOfProvinces(); ++i) {
                        CFG.game.getProvince(CFG.game.getCiv(CFG.game.getProvince(CFG.game.getActiveProvinceID()).getCivID()).getProvinceID(i)).setFromCivID(0);
                    }
                    this.setVisible(false);
                    CFG.toast.setInView(CFG.langManager.get(CFG.gameAutonomy.getAutonomy(toIdeology).getName()), CFG.COLOR_INGAME_GOLD);
                    return;

                } else if (DiplomacyManager.changeAutonomyType(CFG.game.getProvince(CFG.game.getActiveProvinceID()).getCivID(), toIdeology)) {
                    CFG.toast.setInView(CFG.langManager.get(CFG.gameAutonomy.getAutonomy(toIdeology).getName()), CFG.COLOR_INGAME_GOLD);
                }
                ...
                CFG.menuManager.updateInGame_TOP_All(CFG.game.getProvince(CFG.game.getActiveProvinceID()).getCivID());
