        ...
        switch (iID) {
            ...
            default: {
                if (iID % 2 == 1) {
                    ...
                    try {
                        try {
                            final FileHandle file = Gdx.files.local("game/leadersRandom/" + this.lTags.get((iID - 1) / 2));
                            CFG.leader_GameData = (Leader_GameData)CFG.deserialize(file.readBytes());
                        }
                        catch (final GdxRuntimeException ex) {
                            final FileHandle file = Gdx.files.internal("game/leadersRandom/" + this.lTags.get((iID - 1) / 2));
                            CFG.leader_GameData = (Leader_GameData)CFG.deserialize(file.readBytes());
                        }
                    }
