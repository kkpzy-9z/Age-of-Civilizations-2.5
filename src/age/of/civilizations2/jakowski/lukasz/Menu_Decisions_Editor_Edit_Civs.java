...
class Menu_Decisions_Editor_Edit_Civs extends SliderMenu {
   private List lFlags = new ArrayList();

   protected Menu_Decisions_Editor_Edit_Civs() {
      super();
      ...
      for(int i = 0; i < CFG.civDecision_GameData.getCivsSize(); ++i) {
         menuElements.add(new Button_NewGameStyle_Left(CFG.langManager.getCiv(CFG.civDecision_GameData.getCiv(i)), CFG.PADDING * 3 + CFG.CIV_FLAG_WIDTH, CFG.PADDING + 2, tY, tempW - 2 - CFG.PADDING * 2 - (int)((float)CFG.BUTTON_HEIGHT * 0.6F), (int)((float)CFG.BUTTON_HEIGHT * 0.6F), true));
      ...
      this.initMenu(new SliderMenuTitle((String)null, CFG.BUTTON_HEIGHT * 3 / 4, false, false) {
         protected void draw(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
            ImageManager.getImage(Images.new_game_top_edge_title).draw2(oSB, Menu_Decisions_Editor_Edit_Civs.this.getPosX() - 2 + iTranslateX, Menu_Decisions_Editor_Edit_Civs.this.getPosY() - ImageManager.getImage(Images.new_game_top_edge_title).getHeight() - this.getHeight(), Menu_Decisions_Editor_Edit_Civs.this.getWidth() + 2, this.getHeight(), false, false);
            ...
            ImageManager.getImage(Images.gradient).draw(oSB, Menu_Decisions_Editor_Edit_Civs.this.getPosX() + iTranslateX, Menu_Decisions_Editor_Edit_Civs.this.getPosY() - ImageManager.getImage(Images.gradient).getHeight() - this.getHeight() * 3 / 4, Menu_Decisions_Editor_Edit_Civs.this.getWidth(), this.getHeight() * 3 / 4, false, true);
            oSB.setColor(new Color(0.0F, 0.0F, 0.0F, 0.65F));
            ImageManager.getImage(Images.gradient).draw(oSB, Menu_Decisions_Editor_Edit_Civs.this.getPosX() + iTranslateX, Menu_Decisions_Editor_Edit_Civs.this.getPosY() - ImageManager.getImage(Images.gradient).getHeight() - CFG.PADDING, Menu_Decisions_Editor_Edit_Civs.this.getWidth(), CFG.PADDING, false, true);
            oSB.setColor(new Color(0.451F, 0.329F, 0.11F, 1.0F));
            ImageManager.getImage(Images.pix255_255_255).draw(oSB, Menu_Decisions_Editor_Edit_Civs.this.getPosX() + iTranslateX, Menu_Decisions_Editor_Edit_Civs.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight(), Menu_Decisions_Editor_Edit_Civs.this.getWidth());
            oSB.setColor(new Color(0.0F, 0.0F, 0.0F, 0.9F));
            ImageManager.getImage(Images.line_32_off1).draw(oSB, Menu_Decisions_Editor_Edit_Civs.this.getPosX() + iTranslateX, Menu_Decisions_Editor_Edit_Civs.this.getPosY() - ImageManager.getImage(Images.pix255_255_255).getHeight() - ImageManager.getImage(Images.line_32_off1).getHeight(), Menu_Decisions_Editor_Edit_Civs.this.getWidth(), 1);
   ...
   protected void actionElement(int iID) {
      ...
      if (iID == 0) {
         CFG.menuManager.setViewID(Menu.eGAME_DECISIONS_EDITOR_EDIT_SELECT_CIVS);
      }

      if ((iID - 1) % 2 == 1) {
         CFG.civDecision_GameData.removeCiv((iID - 1) / 2);
         CFG.menuManager.rebuildDecisions_Edit_Civs();
      } else {
   ...
   private final void loadFlags() {
      ...
      for(int i = 0; i < CFG.civDecision_GameData.getCivsSize(); ++i) {
         try {
            try {
               this.lFlags.add(new Image(new Texture(Gdx.files.internal("game/flags/" + CFG.civDecision_GameData.getCiv(i) + ".png")), Texture.TextureFilter.Nearest));
            } catch (GdxRuntimeException var7) {
               try {
                  this.lFlags.add(new Image(new Texture(Gdx.files.internal("game/flags/" + CFG.ideologiesManager.getRealTag(CFG.civDecision_GameData.getCiv(i)) + ".png")), Texture.TextureFilter.Nearest));
               } catch (GdxRuntimeException var6) {
                  if (CFG.isAndroid()) {
                     try {
                        this.lFlags.add(new Image(new Texture(Gdx.files.local("game/civilizations_editor/" + CFG.civDecision_GameData.getCiv(i) + "/" + CFG.civDecision_GameData.getCiv(i) + "_FL.png")), Texture.TextureFilter.Nearest));
                     } catch (GdxRuntimeException var5) {
                        this.lFlags.add(new Image(new Texture(Gdx.files.internal("game/civilizations_editor/" + CFG.civDecision_GameData.getCiv(i) + "/" + CFG.civDecision_GameData.getCiv(i) + "_FL.png")), Texture.TextureFilter.Nearest));
                     }
                  } else {
                     this.lFlags.add(new Image(new Texture(Gdx.files.internal("game/civilizations_editor/" + CFG.civDecision_GameData.getCiv(i) + "/" + CFG.civDecision_GameData.getCiv(i) + "_FL.png")), Texture.TextureFilter.Nearest));
                  }
