...
extends SliderMenu {
    ...
    protected Menu_InGame_SendInsult(int onCivID) {
        ...
        tY += ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight();
        menuElements.add(new Slider_FlagAction_Date(CFG.langManager.get("SuspendDiplomaticRelations"), CFG.PADDING * 2, ((MenuElement)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElement)menuElements.get(menuElements.size() - 1)).getHeight() + CFG.PADDING, tempWidth - CFG.PADDING * 3 - CFG.BUTTON_WIDTH, CFG.TEXT_HEIGHT + CFG.PADDING * 2 + CFG.PADDING * 4, 15, 100, 25, 0.65f){
        ...
        menuElements.add(new Button_FlagActionSliderStyle(CFG.langManager.get("SendAnInsult"), -1, 2, tY, CFG.BUTTON_WIDTH, true){
            ...
            protected boolean getClickable() {
                return (CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getDiplomacyPoints() >= 2 || CFG.SPECTATOR_MODE);
            }
    ...
    protected final void actionElement(int iID) {
        if (iID == this.getMenuElementsSize() - 1) {
            if (CFG.SPECTATOR_MODE) {
                ...
                CFG.toast.setInView(CFG.langManager.get("Sent") + "!", CFG.COLOR_TEXT_MODIFIER_NEGATIVE);
                CFG.toast.setTimeInView(4500);
                CFG.game.getCiv(this.iOnCivID).getCivilization_Diplomacy_GameData().messageBox.addMessage(new Message_Relations_Insult(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()));
                CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getCivilization_Diplomacy_GameData().removeImproveRelations_WithCivID(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID);
                CFG.game.getCiv(this.iOnCivID).getCivilization_Diplomacy_GameData().removeImproveRelations_WithCivID(this.iOnCivID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID());
                CFG.game.getCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()).getCivilization_Diplomacy_GameData().addEmbeassyClosed(new Civilization_ClosedEmbassy(this.iOnCivID, this.getMenuElement(this.getMenuElementsSize() - 3).getCurrent()));
                CFG.game.getCiv(this.iOnCivID).getCivilization_Diplomacy_GameData().addEmbeassyClosed(new Civilization_ClosedEmbassy(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.getMenuElement(this.getMenuElementsSize() - 3).getCurrent()));
                float out = DiplomacyManager.getDecreaseRelation(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID);
                CFG.game.setCivRelation_OfCivB(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID, CFG.game.getCivRelation_OfCivB(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID) > -100.0F && CFG.game.getCivRelation_OfCivB(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID) + out <= -100.0F ? -99.0F : CFG.game.getCivRelation_OfCivB(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID) + out);
                out *= 0.415F;
                CFG.game.setCivRelation_OfCivB(this.iOnCivID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), CFG.game.getCivRelation_OfCivB(this.iOnCivID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) > -100.0F && CFG.game.getCivRelation_OfCivB(this.iOnCivID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) + out <= -100.0F ? -99.0F : CFG.game.getCivRelation_OfCivB(this.iOnCivID, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID()) + out);
                DiplomacyManager.worldRecations((int)Math.min(30.0F, CFG.game.getCivRelation_OfCivB(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID) + 100.0F) / 3, CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID);
                DiplomacyManager.updateFriendlyCiv(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID);
            } else {
                DiplomacyManager.decreaseRelation(CFG.game.getPlayer(CFG.PLAYER_TURNID).getCivID(), this.iOnCivID, this.getMenuElement(this.getMenuElementsSize() - 3).getCurrent());
